...
Goblin.registerQuest(goblinName, 'onConnection', function* (
  ...
  let commands;
  if (
    zeppelinConfig.allowedCommands &&
    zeppelinConfig.allowedCommands.length > 0
  ) {
  ...
  socket.on('message', function (payload) {
    ...
    if (payload.type === 'QUEST' || payload.type === 'DATA_TRANSFER') {
      if (!payload.data.data.desktopId) {
        ...
        //TODO: enable when no more errors (need to inspect/debug front-end)
        /*socket.terminate();
        disembarkRequest();
        return*/
      }
      if (
        payload.data.data.desktopId &&
        payload.data.data.desktopId !== desktopId
      ) {
        block(resp, {
          data: payload.data,
